...
1)France Insoumise: France Insoumise is a left-wing political movement founded in 2016 by Jean-Luc Mélenchon, a long-time French politician. The movement opposes neoliberalism, globalization, and defends more progressive policies such as higher taxes on the wealthy and the fight against austerity. It defines itself as anticapitalist and promotes greater participatory democracy. At the European level, France Insoumise is affiliated with the European United Left/Nordic Green Left group in the European Parliament.
2)Les Républicains: Les Républicains, previously known as UMP (Union for a Popular Movement), are a center-right party in France. The party was founded in 2002 by a broad coalition of center-right and right-wing parties. Les Républicains support conservative policies including free market principles, reducing public spending, and a stricter immigration policy. At the European level, Les Républicains are affiliated with the European People's Party (EPP), a political group that includes many other European center-right parties.
3)Les Verts: Les Verts, or Europe Écologie-Les Verts (EELV), is an ecologist political party in France. It was founded in 1984 and is the main ecologist party in the country. Les Verts focuses on environmental protection, sustainability, and combating climate change. The party supports policies such as renewable energy, reducing greenhouse gas emissions, and promoting public transportation. At the European level, Les Verts are part of the Greens/European Free Alliance group in the European Parliament.
4)Parti Socialiste: The Socialist Party is a center-left party in France. Founded in 1969, the party has traditionally supported social democratic policies such as social welfare, redistribution of resources, and increased economic regulation. However, over the years, the Socialist Party has undergone various ideological transformations. At the European level, the 5)Socialist Party is affiliated with the Progressive Alliance of Socialists and Democrats group in the European Parliament.
Place Publique: Place Publique is a progressive political movement founded in 2018 by Raphaël Glucksmann, a French intellectual and journalist. The movement focuses on defending democracy, human rights, the environment, and progressive social policies. It was a significant player in the 2019 European elections in France. At the European level, Place Publique is associated with the Progressive Alliance of Socialists and Democrats group in the European Parliament.
6)Rassemblement National: Rassemblement National, previously known as the National Front, is a far-right political party in France. Founded in 1972 by Jean-Marie Le Pen, the party is known for its anti-immigration, eurosceptic, and nationalist positions. It opposes globalization and supports policies of national sovereignty. At the European level, Rassemblement National is associated with the Identity and Democracy group in the European Parliament.
7)Reconquête: Reconquête is a right-wing French political movement founded by Éric Zemmour, a prominent journalist and writer. The movement opposes immigration and promotes traditional conservative policies. Reconquête has gained attention for its positions against multiculturalism and for the defense of French identity. It is not associated with a specific group in the European Parliament.
8)Renaissance: Renaissance is a progressive political movement founded by French President Emmanuel Macron in view of the 2019 European elections. The movement supports liberal, pro-European, and reformist policies. It focuses on modernizing the French economy, promoting European solidarity, and combating climate change. At the European level, Renaissance is associated with the Renew Europe group in the European Parliament.
...
Simple syntathic preprocessing: 
-trasformations in lower letters
...
```{r}
#remove columns
tdm_df <- subset(tdm_df, select = -c(celui,des,mati,sol,ceux,voi,sub,tres,not,tout,beaucoup,autr,comm,poch,une,environ,oui,là,ça,entre,encor,jusqu,icip,apres,toujour,non,moin,dont,depuis,plusieur,mieux,déjà,qua,quoi,pou))
...
The dictionary for syuzhet is available for 108 languages and is able to classify positive and negative documents. It is able to classify the text in other 8 sentiment too.
...
CA-GALT
tdm:   
Y: a data frame with n rows (individuals) and p columns (frequencies) parole sulle colonne
X: a data frame with n rows (individuals) and k columns (variables) partiti sulle colonne 

DA RIMUOVERE
```{r}
post <- read_excel("C:/Users/maria/OneDrive/Documents/Mery/scuola/Social media analytics/Elezioni Francia Dati_grezzi.xlsx", sheet = 1)

post$Profile[post$Profile %in% 'FranceInsoumise'] <- 'France_Insoumise'
post$Profile[post$Profile %in%  'Parti Socialiste'] <- 'Parti_Socialiste'
post$Profile[post$Profile %in%  'les Républicains'] <- 'Les_Republicains'
post$Profile[post$Profile %in%  'EELV'] <- 'Les_Verts'
post$Profile[post$Profile %in%  'Place publique'] <- 'Place_Publique'
post$Profile[post$Profile %in%  'RECONQUÊTE'] <- 'Reconquete'
post$Profile[post$Profile %in%  'Rassemblement National'] <- 'Rassemblement_National'
post$Profile[post$Profile %in%  'La France insoumise'] <- 'France_Insoumise'
post$Profile[post$Profile %in%  'partisocialiste'] <- 'Parti_Socialiste'
post$Profile[post$Profile %in%  'lesRepublicains'] <- 'Les_Republicains'
post$Profile[post$Profile %in%  'Europe Écologie-Les Verts'] <- 'Les_Verts'
post$Profile[post$Profile %in%  'placepublique_'] <- 'Place_Publique'
post$Profile[post$Profile %in%  'Reconquete_off'] <- 'Reconquete'
post$Profile[post$Profile %in%  'RNational_off'] <- 'Rassemblement_National'

post$Profile <- ifelse(post$Network == "FACEBOOK", paste0(post$Profile, "_f"),
                     ifelse(post$Network == "TWITTER", paste0(post$Profile, "_t"), post$Profile))
...
#pulizia da fare meglio
post$Message = gsub("’", ",", post$Message)
post$Message = gsub("'", ",", post$Message)
post$Message = gsub("[^\x01-\x7F]", "", post$Message)
post.token = quanteda::tokens(post$Message, what = "word", remove_punct = T, remove_url = T, remove_number = T, remove_symbols = T, remove_separators = T )
post.token = quanteda::tokens_tolower(post.token)
post.token = tokens_select(post.token, stopwords("french"), selection = "remove")
post.token %>% head()

#hashtags 
hashtag_uno = convert(dfm(post.token), to = "data.frame") 
hashtag_uno$Partito = post$Profile
hashtag_uno = hashtag_uno %>%
  group_by(Partito) %>%
...
prima = hashtag_uno %>% as.data.frame() %>% dplyr::select(-Partito)

seconda = hashtag_uno %>% mutate(orientamento = "centro") %>% mutate(orientamento = ifelse(Partito %in% c("pd", "avs"),"sinistra", orientamento))
seconda = seconda %>% mutate(orientamento = ifelse(Partito %in% c("lega", "fi", "fdi"), "destra", orientamento))

seconda = seconda %>% dplyr::select(Partito, orientamento) %>% mutate(Partito = as.factor(Partito), orientamento = as.factor(orientamento))
